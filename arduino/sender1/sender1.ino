/*
 * ============================================================
 *  Smart Factory — LoRa Sender NODE_01
 *  Board  : ESP32 DevKit
 ...
 *  └──────────────┴──────────┴──────────────────┘
 *
 *  TX STAGGER: NODE_01 = 0 ms offset
...
#define TX_STAGGER_MS  0UL        // NODE_01 transmits first — no wait
...
// ================= I2C BUS SCANNER =================
// Scans 0x01–0x7F and prints every responding device.
// MPU6050 should appear at 0x68 (AD0=GND) or 0x69 (AD0=3.3V).
void i2cScan() {
  Serial.println("[NODE_01] --- I2C Bus Scan ---");
  uint8_t found = 0;
  for (uint8_t addr = 1; addr < 127; addr++) {
    Wire.beginTransmission(addr);
    uint8_t err = Wire.endTransmission();
    if (err == 0) {
      Serial.printf("[NODE_01]   Device found at 0x%02X", addr);
  ...
  if (found == 0) {
    Serial.println("[NODE_01]   NO I2C devices found!");
    Serial.println("[NODE_01]   CHECK WIRING:");
    Serial.println("[NODE_01]     MPU6050 SDA  -> ESP32 GPIO 21");
    Serial.println("[NODE_01]     MPU6050 SCL  -> ESP32 GPIO 22");
    Serial.println("[NODE_01]     MPU6050 VCC  -> 3.3V  (NOT 5V!)");
    Serial.println("[NODE_01]     MPU6050 GND  -> GND");
    Serial.println("[NODE_01]     MPU6050 AD0  -> GND   (for addr 0x68)");
    Serial.println("[NODE_01]   Also check for loose jumper wires or cold solder joints.");
  }
  Serial.println("[NODE_01] --- Scan done ---");
}

// ================= MPU6050 RAW WAKE + WHO_AM_I =================
// Clone MPU6050 chips respond to I2C addr 0x68 but have a non-standard
// WHO_AM_I register (0x75), causing Adafruit mpu.begin() to return false.
// Fix: manually wake the chip and lower I2C clock before begin().
bool mpuWakeAndInit(uint8_t addr = 0x68) {
  // Step 1: slow clock — some clones need 100 kHz not 400 kHz
  Wire.setClock(100000);
  delay(10);

  // Step 2: read WHO_AM_I register (0x75) — just for diagnostics
  Wire.beginTransmission(addr);
  Wire.write(0x75);
  Wire.endTransmission(false);
  Wire.requestFrom((uint8_t)addr, (uint8_t)1);
  uint8_t whoami = Wire.available() ? Wire.read() : 0xFF;
  Serial.printf("[NODE_01] MPU6050 WHO_AM_I reg=0x%02X ", whoami);
  if      (whoami == 0x68) Serial.println("(genuine / compatible)");
  else if (whoami == 0x70) Serial.println("(MPU6050C variant — OK)");
  else if (whoami == 0x98) Serial.println("(clone chip — forcing wake)");
  else                     Serial.println("(unknown clone — forcing wake)");

  // Step 3: manually clear sleep bit — write 0x00 to PWR_MGMT_1 (reg 0x6B)
  Wire.beginTransmission(addr);
  Wire.write(0x6B);   // PWR_MGMT_1
  Wire.write(0x00);   // clear SLEEP bit, use internal 8 MHz clock
  Wire.endTransmission();
  delay(100);          // chip needs ~50 ms to stabilize after wake

  // Step 4: try mpu.begin() — for genuine chips
  ...
  // Step 5: fallback — chip is awake (PWR_MGMT_1 already cleared).
  // Configure registers to match what the library would set, then verify
  // by reading ACCEL_XOUT_H (0x3B).

  // Set ACCEL_CONFIG (0x1C) = 0x10 → ±8g range (4096 LSB/g)
  // CRITICAL: default after reset is 0x00 = ±2g (16384 LSB/g).
  // The raw read scale (9.81/4096) only works when this is set to ±8g.
  Wire.beginTransmission(addr);
  ...
  // Set CONFIG (0x1A) = 0x04 → 21 Hz DLPF (matches MPU6050_BAND_21_HZ)
  Wire.beginTransmission(addr);
  ...
  Serial.println("[NODE_01] Raw mode: ACCEL_CONFIG=±8g  DLPF=21Hz");
  ...
  if (err == 0 && Wire.available() == 6) {
    Serial.println("[NODE_01] Clone MPU: library rejected but chip is ALIVE — using raw I2C reads");
    mpuRawMode = true;   // switch readVibMagnitude to direct register reads
    return true;
  }
  return false;
}
...
// ================= MPU6050 =================
// Returns DYNAMIC vibration (deviation from gravity).
// Supports both Adafruit library path and raw I2C path for clone chips.
...
float readVibMagnitude(float &ax_out, float &ay_out, float &az_out) {
  ...
  return fabsf(total - GRAVITY_MS2);   // deviation from gravity = dynamic vib
}
...
void tryInitMpu() {
  if (millis() - lastMpuRetryMs < 5000UL) return;  // throttle to every 5s
  lastMpuRetryMs = millis();
  Serial.println("[NODE_01] *** MPU6050 auto-retry ***");
  i2cScan();
  Wire.end();
  delay(50);
  Wire.begin(21, 22);
  ...
  if (mpuWakeAndInit()) {
    mpuOk = true;
    Serial.println("[NODE_01] MPU6050 recovered!");
  } else {
    Serial.println("[NODE_01] MPU6050 still not responding — vib will report 0");
  }
}
...
void tryInitLora() {
  ...
  Serial.println("[NODE_01] *** LoRa auto-retry ***");
  Serial.println("[NODE_01]   SS=GPIO5  DIO0=GPIO26  RST=GPIO14");
  Serial.println("[NODE_01]   SCK=18  MISO=19  MOSI=23  VCC=3.3V");
  ...
  SPI.endTransaction();
  Serial.printf("[NODE_01] SPI reg 0x42 = 0x%02X", ver);
  ...
  if (state == RADIOLIB_ERR_NONE) {
    loraOk = true;
    radio.standby();
    Serial.println("[NODE_01] LoRa recovered!");
  } else {
    Serial.printf("[NODE_01] Still failing code=%d\n", state);
  }
}
...
void setup() {
  ...
  delay(1000);
  Serial.println("\n=== NODE_01 STARTING ===");
  Serial.println("[NODE_01] Pins: MQ2=34 MQ135=35 FLAME=33 TRIG=25 ECHO=27");
  Serial.println("[NODE_01]       MPU SDA=21 SCL=22  LoRa SS=5 DIO0=26 RST=14");
  ...
  Serial.printf("[NODE_01] Watchdog armed: %d s\n", WDT_TIMEOUT_SEC);
  ...
  Serial.println("[NODE_01] Initialising MPU6050 (SDA=21 SCL=22)...");
  i2cScan();
  if (mpuWakeAndInit()) {
    mpuOk = true;
    Serial.println("[NODE_01] MPU6050 OK");
  } else {
    Serial.println("[NODE_01] *** MPU6050 INIT FAILED — will auto-retry every 5s ***");
  }

#if HAS_GPS
  GPSSerial.begin(9600, SERIAL_8N1, 16, 17);
  Serial.println("[NODE_01] GPS started");
#endif
  ...
  digitalWrite(LORA_RST, LOW);  delay(15);
  digitalWrite(LORA_RST, HIGH); delay(100);
  Serial.println("[NODE_01] SX1278 hardware reset done");
  ...
  for (int attempt = 1; attempt <= 3 && state != RADIOLIB_ERR_NONE; attempt++) {
    state = radio.begin(LORA_FREQ, LORA_BW_KHZ, LORA_SF, LORA_CR,
                        LORA_SYNC, LORA_POWER, LORA_PREAMBLE);
    Serial.printf("[NODE_01] radio.begin() attempt %d code=%d\n", attempt, state);
    if (state != RADIOLIB_ERR_NONE) delay(300);
  }

  if (state == RADIOLIB_ERR_NONE) {
    loraOk = true;
    Serial.println("[NODE_01] SX1278 LoRa OK @ 433 MHz SF7 BW125 CR4/5");
  } else {
    Serial.printf("[NODE_01] LoRa FAILED code=%d — check wiring!\n", state);
  }
  radio.standby();

  for (int i = 0; i < 3; i++) { readStableDistance(); delay(50); }

  Serial.printf("[NODE_01] TX stagger: %lu ms\n", TX_STAGGER_MS);
  Serial.println("[NODE_01] Ready — transmitting every 2 s");
  Serial.println("====================================================");
}
...
void loop() {
  ...
  Serial.printf("[NODE_01] seq=%-4lu | smoke=%4d gas=%4d flame=%d dist=%5.1f vib=%.2f %s\n",
  ...
  if (payload.length() > 250) {
    Serial.println("[NODE_01] ERROR: payload too large");
    return;
  }

  Serial.printf("[NODE_01] TX %u bytes...", payload.length());
