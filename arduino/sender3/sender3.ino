/*
 * ============================================================
 *  Smart Factory — LoRa Sender NODE_03
 *  Board  : ESP32 DevKit
 ...
 *  └──────────────┴──────────┴──────────────────┘
 *
 *  TX STAGGER: NODE_03 = 1333 ms offset
...
#define TX_STAGGER_MS  1333UL     // NODE_03 waits 1333ms before first TX
...
// ================= I2C BUS SCANNER =================
void i2cScan() {
  Serial.println("[NODE_03] --- I2C Bus Scan ---");
  uint8_t found = 0;
  for (uint8_t addr = 1; addr < 127; addr++) {
    Wire.beginTransmission(addr);
    if (Wire.endTransmission() == 0) {
      Serial.printf("[NODE_03]   Device at 0x%02X", addr);
  ...
  if (found == 0) {
    Serial.println("[NODE_03]   NO I2C devices found!");
    Serial.println("[NODE_03]   CHECK: SDA->GPIO21  SCL->GPIO22  VCC->3.3V  GND->GND  AD0->GND");
  }
  Serial.println("[NODE_03] --- Scan done ---");
}
...
bool mpuWakeAndInit(uint8_t addr = 0x68) {
  ...
  Serial.printf("[NODE_03] MPU6050 WHO_AM_I=0x%02X ", whoami);
  ...
  if (mpu.begin(addr, &Wire)) {
    ...
    Serial.println("[NODE_03] MPU6050 OK via Adafruit library");
    return true;
  }
  ...
  Serial.println("[NODE_03] Raw mode: ACCEL_CONFIG=±8g  DLPF=21Hz");
  ...
  if (err == 0 && Wire.available() == 6) {
    Serial.println("[NODE_03] Clone MPU alive — using raw I2C reads");
    mpuRawMode = true;
    return true;
  }
  return false;
...
void tryInitMpu() {
  ...
  Serial.println("[NODE_03] *** MPU6050 auto-retry ***");
  i2cScan();
  Wire.end();
  delay(50);
  Wire.begin(21, 22);
  ...
  if (mpuWakeAndInit()) {
    mpuOk = true;
    Serial.println("[NODE_03] MPU6050 recovered!");
  } else {
    Serial.println("[NODE_03] MPU6050 still not responding — vib will report 0");
  }
}
...
void tryInitLora() {
  ...
  Serial.println("[NODE_03] *** LoRa auto-retry ***");
  Serial.println("[NODE_03]   SS=GPIO5  DIO0=GPIO26  RST=GPIO14");
  Serial.println("[NODE_03]   SCK=18  MISO=19  MOSI=23  VCC=3.3V");
  ...
  SPI.endTransaction();
  Serial.printf("[NODE_03] SPI reg 0x42 = 0x%02X", ver);
  ...
  if (state == RADIOLIB_ERR_NONE) {
    loraOk = true;
    radio.standby();
    Serial.println("[NODE_03] LoRa recovered!");
  } else {
    Serial.printf("[NODE_03] Still failing code=%d\n", state);
  }
}
...
void setup() {
  ...
  delay(1000);
  Serial.println("\n=== NODE_03 STARTING ===");
  Serial.println("[NODE_03] Pins: MQ2=34 MQ135=35 FLAME=33 TRIG=25 ECHO=27");
  Serial.println("[NODE_03]       MPU SDA=21 SCL=22  LoRa SS=5 DIO0=26 RST=14");
  ...
  Serial.printf("[NODE_03] Watchdog armed: %d s\n", WDT_TIMEOUT_SEC);
  ...
  Serial.println("[NODE_03] Initialising MPU6050 (SDA=21 SCL=22)...");
  i2cScan();
  if (mpuWakeAndInit()) {
    mpuOk = true;
    Serial.println("[NODE_03] MPU6050 OK");
  } else {
    Serial.println("[NODE_03] *** MPU6050 INIT FAILED — will auto-retry every 5s ***");
  }

#if HAS_GPS
  GPSSerial.begin(9600, SERIAL_8N1, 16, 17);
  Serial.println("[NODE_03] GPS started");
#endif
  ...
  digitalWrite(LORA_RST, LOW);  delay(15);
  digitalWrite(LORA_RST, HIGH); delay(100);
  Serial.println("[NODE_03] SX1278 hardware reset done");
  ...
  for (int attempt = 1; attempt <= 3 && state != RADIOLIB_ERR_NONE; attempt++) {
    state = radio.begin(LORA_FREQ, LORA_BW_KHZ, LORA_SF, LORA_CR,
                        LORA_SYNC, LORA_POWER, LORA_PREAMBLE);
    Serial.printf("[NODE_03] radio.begin() attempt %d code=%d\n", attempt, state);
    if (state != RADIOLIB_ERR_NONE) delay(300);
  }

  if (state == RADIOLIB_ERR_NONE) {
    loraOk = true;
    Serial.println("[NODE_03] SX1278 LoRa OK @ 433 MHz SF7 BW125 CR4/5");
  } else {
    Serial.printf("[NODE_03] LoRa FAILED code=%d — check wiring!\n", state);
  }
  radio.standby();

  for (int i = 0; i < 3; i++) { readStableDistance(); delay(50); }

  Serial.printf("[NODE_03] TX stagger: %lu ms\n", TX_STAGGER_MS);
  Serial.println("[NODE_03] Ready — transmitting every 2 s");
  Serial.println("====================================================");
}
...
void loop() {
  ...
  Serial.printf("[NODE_03] seq=%-4lu | smoke=%4d gas=%4d flame=%d dist=%5.1f vib=%.2f %s\n",
  ...
  if (payload.length() > 250) {
    Serial.println("[NODE_03] ERROR: payload too large");
    return;
  }

  Serial.printf("[NODE_03] TX %u bytes...", payload.length());
